...
public class AvcAppendEncoder
{
    private final static String TAG = "AvcAppendEncoder";
    ...
    public ArrayBlockingQueue<MyFrame> YUVQueue;

    public DecoderInterface decoderInterface;

    private MediaCodec.BufferInfo audioEncodeBufferInfo;

    public AvcAppendEncoder(int width, int height, int framerate, int bitrate, String path) {
        this.path = path;
        m_width  = width;
        m_height = height;
    ...
    public void putVideoFrameToCodec(MyFrame frame){
        byte[] yuv420sp = new byte[m_width * m_height * 3 / 2];
        int offset = 0;
        ImageUtils.yuvSetBackgroud(yuv420sp, m_width, m_height, backgroundColor);
        ImageUtils.overlayYUV(yuv420sp, offset, m_width, m_height, frame.getBytes(), frame.getWidth(), frame.getHeight());
    ...
    class VideoThread extends Thread{
        ...
        public void run() {
            ...
            while (isRuning) {
                ...
                if (null != YUVQueue && YUVQueue.size() >0) {
                    MyFrame frame = YUVQueue.poll();
                    putVideoFrameToCodec(frame);
                }
